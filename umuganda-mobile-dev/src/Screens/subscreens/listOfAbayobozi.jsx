  ...
  const getIsiboList = async (token) => {
    ...
    try {
      const response = await axios.get(
        "https://umudugudu-backend.onrender.com/api/user/leaders",
        {
          headers: {
      ...
      console.log(
        response.data.data.abayobozi,
        "++++++"
      );
      setList(response.data.data.abayobozi);
      setLoading(false);
    } catch (err) {
      setLoading(false);
      console.log(err, "====");
...
//   const [updatedUsers, setUpdatedUsers] = useState(users);
  ...
  const HandleEditUser = async (userId) => {
    ...
    try {
      const response = await axios.put(`https://umudugudu-backend.onrender.com/api/user/update/${userId}`, updatedUser);
      ...
      <ScrollView>
        <View style={styles.mainContainer}>
          ...
          <View style={styles.bigcontainer}>
            ...
            <Entypo name="edit" size={24} color="black" style={{ marginLeft: 90 }} />
          </TouchableOpacity>
